...
// 401
func StatusNotAuthorized(w http.ResponseWriter, r *http.Request, message string) {
	data := map[string]interface{}{"error": message}
	newResponse(data, http.StatusUnauthorized).sendResponse(w, r)
}
...
func StatusMethodNotAllowed(w http.ResponseWriter, r *http.Request) {
	newResponse("method not allowed", http.StatusMethodNotAllowed).sendResponse(w, r)
}
